...
func TestExamplesComplete(t *testing.T) {
	...
	testStructure.RunTestStage(t, "apply", func() {
		...
		terraformOptions.TerraformDir = tempTestFolder + "/cloudtrail"

		// Run `terraform output` to get the value of an output variable
		bucketARN := terraform.Output(t, terraformOptions, "bucket_arn")

		// Verify we're getting back the outputs we expect
		assert.NotEmpty(t, bucketARN, "Expected `bucketARN` to be populated")
	})
}
...
func TestExamplesCompleteDisabled(t *testing.T) {
	...
	rootFolder := "../../"
	terraformFolderRelativeToRoot := "examples/complete/cloudtrail"
